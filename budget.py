...
budget_sales_groupped_by_item = budget_sales_with_exchange_rates.groupby(by=[
                                                                         "Nr articolo"], as_index=False).sum()
...
budget_sales_groupped_by_item['Mix (%)'] = budget_sales_groupped_by_item['Quantità'] / \
    budget_sales_groupped_by_item['Quantità'].sum()
...
budget_raw_materials_cost = budget_raw_materials_usage.groupby(
    by=["Nr articolo"], as_index=False).sum().rename(columns={"Importo costo (TOTALE)": "Costo MP (€)"})[['Nr articolo', 'Costo MP (€)']]
...
print(budget_item[['Nr articolo', 'Quantità',
